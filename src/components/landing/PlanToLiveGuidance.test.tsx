...
describe('PlanToLiveGuidance', () => {
  ...
  it('starts with the coach introduction, preserves the requested chapter order, and exposes a light theme', () => {
    ...
    expect(chapterButtons[0]).toHaveAccessibleName(/meet your coach iris and reed/i);
  ...
  it('includes missed-workout continuity as the fifth carousel stage', async () => {
    ...
    expect(screen.getByRole('button', { name: /meet your coach iris and reed/i })).toHaveClass('is-complete');
  });
  ...
  it('resets the six-second progress interval after manual selection', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /meet your coach iris and reed/i }));
  ...
  it('introduces the coaches without offering a session to start', () => {
    render(<PlanToLiveGuidance />);

    expect(screen.getByText(/iris and reed coach in different styles/i)).toBeInTheDocument();
