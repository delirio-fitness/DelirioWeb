---
name: delirio-assets
description: Find, pull and create Delirio's shared visual assets — coach art (Reed, Iris), logos, icons, backgrounds, marketing imagery, app screenshots, Lottie animations. Use when a task needs a brand or character asset, when a screenshot of a Delirio app screen is needed for marketing or an App Store listing, when an asset is missing or looks wrong or out of date, when adding an image to this repo, when checking whether committed assets are still current, or when the catalog has nothing suitable and a new asset must be generated.
---
...
- Filters: `--kind` `--subject` `--variant` `--treatment` `--format` `--status`
  `--review`. `kind` is one of
  `coach | icon | logo | background | marketing | screenshot`;
...
### `kind: screenshot` is the one kind where `current` is a claim about the past

Every other kind is true indefinitely — a coach render depicts Reed, and it will
still depict Reed next year. A screenshot depicts a **screen**, and the screen
changes whenever someone restyles a card, renames a label, or ships a new state.

**The asset stays `status: current` and keeps passing every check the whole time
it is drifting out of date.** Nothing in the catalog can see the app, so there is
no signal to wait for and no alert that will reach you. Two consequences you have
to carry yourself:

- **`check` reporting `ok` on a screenshot means the bytes match what was
  recorded, not that the screen still looks like that.** It is the one place
  where a clean `check` is not the reassurance it is everywhere else.
- **Before shipping one somewhere public — an App Store listing, a landing page,
  an ad — say when it was captured and offer a re-capture.** `show <id>` reports
  `provenance.imported`. A months-old capture is not automatically wrong, but the
  decision belongs to a person, and it is cheap: re-capturing keeps the same ID
  and every lockfile pin, so nothing you have pinned breaks.

Assets that bake in a date age fastest — `screenshot_profile_v1` renders the real
capture date in its week strip, and its own description says to re-capture rather
than ship a stale month. **Read the description; these say what they are wrong
for more often than any other kind.**

### Every screen is catalogued twice — bare and framed. Pick deliberately.

They are two assets, not two renditions of one, and taking the wrong one is the
most likely mistake here:

| | `screenshot_<screen>_v1` | `screenshot_<screen>-framed_v1` |
| --- | --- | --- |
| Size | 1206x2622 | 1350x2760 |
| Ground | fully opaque, full-bleed | **transparent** outside the phone |
| Device body | none | iPhone 17 Pro |
| Status bar | none — the app renders none | drawn, reading **9:41** |
| Right for | App Store uploads; compositing into your own frame | drop-in marketing, decks, web |

Two traps in that table:

- **The framed one's status bar is synthesized, not captured.** The 9:41 is drawn
  in. It is wrong anywhere a real device state is required, and wrong anywhere a
  different device is required — you cannot re-frame it onto another body.
  (`screenshot_imessage-coach-thread_v1` is the exception: it is a real device
  capture, so its framed form keeps the status bar it was taken with.)
- **App Store Connect wants the bare one.** Uploading a framed screenshot puts a
  picture of a phone inside a picture of a phone, and the transparent ground will
  be flattened to something you did not choose.
...
- **Screenshots.** `generate --kind screenshot` is **refused**, and the refusal
  is the point: a screenshot is a capture of the real app, and a model asked for
  one produces a convincing picture of a product that does not exist — carrying a
  permanent ID, indistinguishable from the real captures once it is in the
  catalog. A screen that is missing or out of date is captured from a running
  build, dropped into `Library/Screenshots/` in the byte store, and picked up by
  `delirio-assets ingest` in the catalog checkout. Say the screen is missing and
  hand it over; do not render one.
